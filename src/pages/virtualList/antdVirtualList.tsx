...
const AntdVirtualList: React.FC = () => {
  ...
  function getExInfo(map: any) {
    _.keys(map).forEach((key: string) => {
      if (map[key].date.length > 0) {
        ...
        const startTime = item.sort(
          (a: any, b: any) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime(),
        )[0].startTime;
        const endTime = item.sort((a: any, b: any) => new Date(b.endTime).getTime() - new Date(a.endTime).getTime())[0]
          .endTime;
  ...
  return (
    <List>
      <VirtualList data={data} height={ContainerHeight} itemHeight={47} itemKey="id" onScroll={onScroll}>
        {(item: any) => {
          const isFirstLevel = !item.rootSetId && !item.parentSetId;
